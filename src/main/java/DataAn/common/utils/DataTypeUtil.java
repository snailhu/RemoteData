...
public class DataTypeUtil {
	...
	public static Object invokeMethod(String className, String methodName,
		...
		try {
			...
			if (cls == null)
				return obj;
			Constructor classConstructor = cls.getConstructor();

			Object clsObj = classConstructor.newInstance();
			// 有参数类型，直接调用
			if (argsType != null) {
			...
			LogUtil.getInstance().getLogger(DataTypeUtil.class).error(ex.getMessage(), ex);
			return null;
		}
	}
	...
	public static Object[] getSortedKey(Hashtable ht) {
		...
		try {
			...
			LogUtil.getInstance().getLogger(DataTypeUtil.class).error("sort error", ex);
	...
	public static int bytesCopy(byte[] source, byte[] dest, int len) {
		try {
			...
			LogUtil.getInstance().getLogger(DataTypeUtil.class).error(
	...
	public static String MD5(String src) {
		...
		try {
			...
			LogUtil.getInstance().getLogger(DataTypeUtil.class).error("MD5 error", e);
		}
	...
	public static String MD5UTF8(String src) {
		...
		try {
			...
			LogUtil.getInstance().getLogger(DataTypeUtil.class).error("MD5 utf-8 error", e);
			e.printStackTrace();
		}
		byte[] b = md.digest();
		des = bytes2Hexs(b);
	...
	public static Hashtable sortHashTable(Hashtable ht) {
		...
		try {
			...
			// ht.put("02", "增值税小规模");
			Vector v = new Vector(ht.keySet());
			// zxt.pub.util.LogUtil.getInstance().getLogger(DataTypeUtil.class).debug(v);
			Collections.sort(v);
			Object[] keys = v.toArray();
			int len = ht.size();
	...
	public static void main(String[] args) {
		...
		System.out.println(DataTypeUtil.isNotEmpty(objs));
	}
}
