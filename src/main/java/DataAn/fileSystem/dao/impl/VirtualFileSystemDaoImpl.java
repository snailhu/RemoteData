...
implements IVirtualFileSystemDao{
	...
	public VirtualFileSystem selectByFileName(String fileName) {
		...
		List<VirtualFileSystem> list= this.getSession().createQuery(hql).setParameter(0, fileName).list();
		if(list != null && list.size() > 0){
			return list.get(0);
		}
		return null;
	...
	@SuppressWarnings("unchecked")
	@Override
	public VirtualFileSystem selectByParentIdAndFileNameAndParameterType(long parentId,
		...
		if(parentId == 0){
			...
			List<VirtualFileSystem> list = this.getSession().createQuery(hql).setParameter(0, fileName).setParameter(1, parameterType).list();
			if(list != null && list.size() > 0){
				return list.get(0);
			}
		}else{
