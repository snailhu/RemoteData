...
public class ChartUtils {
	...
	public static DefaultCategoryDataset createDefaultCategoryDataset(
			Serie serie, Vector<String> categories) {
		DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		String name = serie.getName();
		Vector<Object> data = serie.getData();
		if (data != null && categories != null && data.size() == categories.size()) {
			for (int index = 0; index < data.size(); index++) {
				...
				if (isNumber(value)) {
					dataset.setValue(Double.parseDouble(value), name, categories.get(index));
				}
			}
		}
		return dataset;
