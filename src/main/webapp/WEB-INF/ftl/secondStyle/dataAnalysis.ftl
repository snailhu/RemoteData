		 ...
		 <div class="hr hr32 hr-dotted"></div>
			...
			<div class="col-xs-12">
					<div style="margin-left:1px;margin-top:0px" id='jqxButton_addgroup'>添加分组</div>
  					...
  					<div style="margin-left:220px;margin-top:-26px" id="id_dplist_template"></div>
  		 	</div>
  		 	<div class="col-xs-12">	
  				<div id='jqxWidgett'>
			        <div id="treeGrid"></div>			       	
				...
				.on(
						...
						function(event) {
							if (event.args) {
								...
								if (item) {
									var url = "${base}/getParamsByTemplateId?templateId="
											+ item.value;
									updateParamTree(url);
								}
							}
							...
							columns : [
									...
									},
									{
										...
										cellsRenderer : function(row, column,
												value, rowData) {
											if (value == "添加到分组") {
												return "<input type='button' value='删除模板'></input>"
											}
										},
