...
<!-- 		</div> -->
			    ...
			    <div class="modal-content">
			      ...
			      <div class="modal-footer">
			      	<div class="col-lg-4 col-lg-offset-5">
				        <button type="button" class="btn btn-default" id="add-role-close" data-dismiss="modal">关闭</button>
				        <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="submit_addRoleInfo()">确定</button>
                    </div>
			      </div>
			    </div>
			  </div>
			  ...
			  <div class="modal-dialog" role="document" style="margin:55px -300px">
			    <div class="modal-content">
			      ...
			      <div class="modal-footer">
			      	<div class="col-lg-4 col-lg-offset-5">
				        <button type="button" class="btn btn-default" id="add-role-close" data-dismiss="modal">关闭</button>
				        <button type="button" class="btn btn-primary" data-dismiss="modal" id="submit_editRoleInfo">确定</button>
                    </div>
			      </div>
			    </div>
			  </div>
			  ...
			  <div class="modal-dialog" role="document" style="margin:55px -300px">
			    <div class="modal-content">
			      ...
			      <div class="modal-footer">
			      	<div class="col-lg-4 col-lg-offset-5">
				        <button type="button" class="btn btn-default" id="add-role-close" data-dismiss="modal">关闭</button>
				        <button type="button" class="btn btn-primary" data-dismiss="modal" id="submit_editRolePermission">确定</button>
                    </div>
			      </div>
			    </div>
			  </div>
	...
	<script type="text/javascript">
	  ...
	  //创建系统角色
	  function submit_addRoleInfo(){
		var name = $('#add-role-name').val();
		var description = $('#add-role-description').val();
		$('#add-role-name').val("");
		$('#add-role-description').val("");
		//$('#addPermissionGroupInfoForm').data('bootstrapValidator').resetForm(true);
		$.ajax({
				url : '${pageContext.request.contextPath}/admin/role/createRole',
				data : {
					name : name,
	          ...
	          if (rows.length == 1) {
				...
				$('#submit_editRoleInfo').click(function(){
					...
					if(oldName != name || oldDescription != description){
						$.ajax({
							url : '${pageContext.request.contextPath}/admin/role/editRole',
							data : {
								id : rows[0].id,
								name : name,
								description : description
