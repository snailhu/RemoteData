...
;(function(root, factory) {
    ...
    var jeDt = {}, doc = document, dtCell = [ "jedatebox" ], ymdMacth = /\w+|d+/g;
	...
	*/
    ...
    jeDt.parse = function(ymd, hms, format) {
        ymd = ymd.concat(hms);
        var format = format, hmsCheck = jeDt.parseCheck(format, false).substring(0, 5) == "hh:mm", num = 2;
    ...
    jeDt.setHtml = function(opts, self, lyerCell) {
        var that = this, weekHtml = "", tms = "", date = new Date(), nowDateVal = jeDt.nowDate(null, opts.format || config.format), 
		    isYYMM = jeDt.parseCheck(opts.format || config.format, true) == "YYYY-MM" ? true :false, ishhmm = jeDt.parseCheck(opts.format || config.format, false).substring(0, 5) == "hh:mm" ? true :false, 
			isTimeFormat = !isYYMM ? true :false, isTime = opts.isTime, 
        ...
        if ((jeDt.val(self) || jeDt.text(self)) != "") {
            var inVals = initVal.match(/\d+/g), parseFormat = jeDt.parseCheck(opts.format || config.format, false);
            tms = parseFormat.substring(0, 5) == "hh:mm" ? parseFormat == "hh:mm" ? [ inVals[0], inVals[1], date.getSeconds() ] :[ inVals[0], inVals[1], inVals[2] ] :[ inVals[0], inVals[1], inVals[2], inVals[3], inVals[4], inVals[5] == undefined ? date.getSeconds() :inVals[5] ];
        } else {
            tms = [ date.getFullYear(), date.getMonth() + 1, date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds() ];
        }
        //控件HMTL模板
        ...
        var bothmsStr = !isYYMM ? '<div class="botflex jedatehmsshde"><ul class="jedatehms"><li><em></em><i>:</i></li><li><em></em><i>:</i></li><li><em></em></li></ul></div>' + '<div class="botflex jedatebtn"><span class="jedatetodaymonth">今天</span><span class="jedateclear">清空</span></div>' :'<div class="botflex jedatebtn"><span class="jedatetodaymonth">本月</span><span class="jedateclear">清空</span></div>';
        var datebotStr = '<div class="jedatebot">' + bothmsStr + "</div>";
        var datehmschoose = '<div class="jedateprophms ' + (ishhmm ? "jedatepropfix" :"jedateproppos") + '" prop="true"><div class="jedatepropcon"><div class="jedatehmstitle">时分秒选择<div class="jedatehmsclose">&times;</div></div><div class="jedateproptext">小时</div><div class="jedateproptext">分钟</div><div class="jedateproptext">秒数</div><div class="jedatehmscon jedateprophours"></div><div class="jedatehmscon jedatepropminutes"></div><div class="jedatehmscon jedatepropseconds"></div></div></div>';
        ...
        if (!isYYMM) {
            ...
            ishhmm ? jeDt.getDateStr(opts, lyerCell, date.getFullYear(), date.getMonth() + 1, date.getDate()) :jeDt.getDateStr(opts, lyerCell, tms[0], tms[1], tms[2]);
    ...
    jeDt.onlyYMStr = function(opts, y, m) {
        ...
        jeDt.each(jeDt.montharr, function(i, val) {
            var minArr = jeDt.parseMatch(opts.minDate || config.minDate), maxArr = jeDt.parseMatch(opts.maxDate || config.maxDate), thisDate = new Date(y, jeDt.digit(val), "01"), minTime = new Date(minArr[0], minArr[1], minArr[2]), maxTime = new Date(maxArr[0], maxArr[1], maxArr[2]);
    ...
    jeDt.onlyYMevents = function(opts, self, lyerCell, tms) {
        var ymPre = QD(lyerCell + " .jedateym .ymprev"), ymNext = QD(lyerCell + " .jedateym .ymnext"), ony = parseInt(tms[0]), onm = parseFloat(tms[1]);
    ...
    jeDt.orien = function(obj, self, pos) {
        ...
        tops = rect.bottom + obj.offsetHeight / 1.5 <= jeDt.winarea() ? rect.bottom - 1 :rect.top > obj.offsetHeight / 1.5 ? rect.top - obj.offsetHeight + 1 :jeDt.winarea() - obj.offsetHeight;
    ...
    jeDt.getDateStr = function(opts, lyerCell, y, m, d) {
        ...
        //判断是否超出允许的日期范围	
        var startDay = 1, minArr = jeDt.parseMatch(opts.minDate || config.minDate), maxArr = jeDt.parseMatch(opts.maxDate || config.maxDate), 
		    endDay = currentMonthDays, thisDate = new Date(y, m, d), firstDate = new Date(y, m, 1), lastDate = new Date(y, m, currentMonthDays), 
    ...
    jeDt.getStrhms = function(opts, lyerCell, hmsArr) {
        ...
        jeDt.each([ 24, 60, 60 ], function(i, len) {
            var hmsStr = "", hmsCls = "", textem = jeDt.text(QD(lyerCell + " .jedatehms em")[i]), parseFormat = parsehms(opts.format || config.format);
            for (var h = 0; h < len; h++) {
                h = jeDt.digit(h);
                if (opts.ishmsLimit) {
                    hmsCls = h < textem ? "disabled" :h == textem ? "action" :"";
                } else {
                    hmsCls = parseFormat == "hh:mm" && i == 2 ? textem == h ? "disabled action" :"disabled" :textem == h ? "action" :"";
                }
    ...
    jeDt.events = function(opts, self, lyerCell, tms) {
        var newDate = new Date(), yPre = QD(lyerCell + " .yearprev"), yNext = QD(lyerCell + " .yearnext"), 
			...
			ishhmmss = jeDt.parseCheck(opts.format || config.format, false).substring(0, 5) == "hh:mm" ? true :false;
        if (!isYYMM) {
            //切换年
            ...
            //时分秒事件绑定
            var hmsStr = jeDt.getStrhms(opts, lyerCell), hmsevents = function(hmsArr) {
                ...
                jeDt.each([ "hours", "minutes", "seconds" ], function(i, hms) {
                    ...
                    jeDt.bind(onhmsPCls, "click", function() {
                        ...
                        if (!ishhmmss) jeDt.isShow(QD(lyerCell + " .jedateprophms")[0], false);
                    });
                });
            };
            ...
            jeDt.bind(QD(lyerCell + " .jedatebot .jedatetodaymonth"), "click", function() {
                var toTime = [ newDate.getFullYear(), newDate.getMonth() + 1, newDate.getDate(), newDate.getHours(), newDate.getMinutes(), newDate.getSeconds() ], 
				    gettoDate = jeDt.parse([ toTime[0], toTime[1], toTime[2] ], [ toTime[3], toTime[4], toTime[5] ], opts.format || config.format), valcell = opts.isDisplay ? QD("body " + lyerCell.replace(/\#/g, "."))[0] :self;
        ...
        jeDt.bind(QD(lyerCell + " .jedatebot .jedateok"), "click", function(ev) {
            jeDt.stopmp(ev);
            var valcell = opts.isDisplay ? QD("body " + lyerCell.replace(/\#/g, "."))[0] :self, 
			isValtext = (jeDt.val(valcell) || jeDt.text(valcell)) != "", 
            ...
            if (isValtext) {
                var jedCell = isYYMM ? QD(lyerCell + " .jedaym li") :QD(lyerCell + " .jedaul li"), 
				    nowDateVal = jeDt.nowDate(null, opts.format || config.format), 
                ...
                eachhmsem(okArr);
                if (ishhmmss) {
                    okVal = jeDt.parse([ tms[0], tms[1], tms[2] ], [ okArr[0], okArr[1], okArr[2] ], opts.format || config.format);
                } else {
                    var okDate = [ newDate.getFullYear(), newDate.getMonth() + 1, newDate.getDate(), okArr[0], okArr[1], okArr[2] ], 
					    okVal = jeDt.parse([ okDate[0], okDate[1], okDate[2] ], [ okDate[3], okDate[4], okDate[5] ], opts.format || config.format);
                }
                jeDt.isValHtml(valcell) ? jeDt.val(valcell, okVal) :jeDt.text(valcell, okVal);
                jeDt.dateClose(opts, lyerCell);
            }
    ...
    jeDt.chooseYM = function(opts, self, lyerCell, tms) {
        var jetopym = QD(lyerCell + " .jedatetopym"), jedateyy = QD(lyerCell + " .jedateyy"), jedatemm = QD(lyerCell + " .jedatemm"), jedateyear = QD(lyerCell + " .jedateyy .jedateyear"), 
		    jedatemonth = QD(lyerCell + " .jedatemm .jedatemonth"), mchri = QD(lyerCell + " .jedateymchri"), mchle = QD(lyerCell + " .jedateymchle"), 
		    ishhmmss = jeDt.parseCheck(opts.format || config.format, false).substring(0, 5) == "hh:mm" ? true :false;
        //循环生成年
